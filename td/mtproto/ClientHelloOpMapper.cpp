...
vector<Op> make_extension_ops(const BrowserProfile &profile, const BrowserExtension &extension) {
  vector<Op> ops;
  ...
  switch (extension.type) {
    ...
    case TlsExtensionType::EncryptedClientHello:
      return {Op::bytes(store_u16(type)), Op::scope16_begin(), Op::bytes("\x00\x00\x01\x00\x01"), Op::random_bytes(1),
              Op::bytes("\x00\x20"), Op::random_bytes(32), Op::scope16_begin(), Op::random_bytes(144),
              Op::scope16_end(), Op::scope16_end()};
    default:
      return {Op::bytes(store_u16(type)), Op::scope16_begin(), Op::bytes(extension.raw_data), Op::scope16_end()};
  }
...
vector<vector<Op>> make_extension_parts(const BrowserProfile &profile) {
  vector<vector<Op>> result;
  result.reserve(profile.extensions.size());
  for (auto &extension : profile.extensions) {
    result.push_back(make_extension_ops(profile, extension));
  }
  return result;
}

vector<Op> expand_template(const BrowserProfile &profile) {
  vector<Op> result;
  for (auto &op : profile.layout_template) {
    switch (op.type) {
      ...
      case Op::Type::ExtensionsFromProfile: {
        for (auto &part : make_extension_parts(profile)) {
          result.insert(result.end(), part.begin(), part.end());
        }
        break;
      }
      case Op::Type::ExtensionsPermutationFromProfile:
        result.push_back(Op::permutation(make_extension_parts(profile)));
        break;
      default:
        result.push_back(op);
        break;
...
vector<ClientHelloOp> ClientHelloOpMapper::map(const BrowserProfile &profile) {
  return expand_template(profile);
}
