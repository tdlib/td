...
ForumTopicInfo::ForumTopicInfo(Td *td, const telegram_api::object_ptr<telegram_api::ForumTopic> &forum_topic_ptr,
  ...
  dialog_id_ = DialogId(forum_topic->peer_);
  ...
  if (creator_dialog_id_.is_valid() && creator_dialog_id_.get_type() != DialogType::User) {
    td->dialog_manager_->force_create_dialog(creator_dialog_id_, "ForumTopicInfo creator", true);
  }
  ...
  if (expected_dialog_id != DialogId() && dialog_id_ != expected_dialog_id) {
    LOG(ERROR) << "Receive forum topic in " << dialog_id_ << " instead of " << expected_dialog_id << ": "
               << to_string(forum_topic_ptr);
    *this = ForumTopicInfo();
  }
}
