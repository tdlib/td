...
void DialogManager::on_failed_public_dialogs_search(const string &query, DialogTypeFilter type_filter, Status &&error) {
  ...
  found_public_dialogs_[type_num].emplace(query, vector<DialogId>());     // negative cache
  found_on_server_dialogs_[type_num].emplace(query, vector<DialogId>());  // negative cache
