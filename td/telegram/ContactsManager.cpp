...
Status ContactsManager::can_manage_chat_join_requests(DialogId dialog_id) {
  if (!td_->dialog_manager_->have_dialog_force(dialog_id, "can_manage_chat_join_requests")) {
  ...
  switch (dialog_id.get_type()) {
    ...
    case DialogType::Chat: {
      ...
      if (!get_chat_is_active(chat_id)) {
        return Status::Error(400, "Chat is deactivated");
      }
      if (!get_chat_status(chat_id).can_manage_invite_links()) {
        return Status::Error(400, "Not enough rights to manage chat join requests");
      }
      break;
    }
    case DialogType::Channel:
      if (!get_channel_status(dialog_id.get_channel_id()).can_manage_invite_links()) {
        return Status::Error(400, "Not enough rights to manage chat join requests");
      }
      break;
    case DialogType::None:
...
void ContactsManager::get_dialog_join_requests(DialogId dialog_id, const string &invite_link, const string &query,
  ...
  TRY_STATUS_PROMISE(promise, can_manage_chat_join_requests(dialog_id));
