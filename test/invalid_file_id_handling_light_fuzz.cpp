...
TEST(InvalidFileIdHandlingLightFuzz, randomized_literal_sampling_preserves_invalid_file_id_guards) {
  ...
  const std::array<SnippetCase, 7> cases = {{
      ...
      {1,
       "autofile_upload_ids=transform(file_ids,[](FileIdfile_id){returnFileUploadId(file_id,"
       "FileManager::get_internal_upload_id());});",
       false},
      {1, "if(pos<file_upload_ids_.size()&&pos<file_references_.size()&&!was_uploaded_){", false},
  }};
  ...
  for (int i = 0; i < kIterations; i++) {
    ...
    const auto &source = test_case.source_kind == 0 ? normalized_message_content : normalized_messages_manager;
