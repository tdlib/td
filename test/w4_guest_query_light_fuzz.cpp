...
TEST(W4GuestQueryLightFuzz, RandomizedProbeOrderKeepsRequiredGuestPatternsPinned) {
  ...
  const std::array<td::Slice, 34> required = {
      ...
      td::Slice("promise_.set_value(td_api::make_object<td_api::inlineMessageId>(InlineQueriesManager::get_inline_"
                "message_id(std::move(ptr))));"),
...
TEST(W4GuestQueryLightFuzz, RandomizedProbeOrderKeepsLegacyUnsafePatternsAbsent) {
  auto td_api_tl = read_normalized("td/generate/scheme/td_api.tl");
  auto updates_manager = read_normalized("td/telegram/UpdatesManager.cpp");
  auto inline_queries_manager = read_normalized("td/telegram/InlineQueriesManager.cpp");
  auto messages_manager_h = read_normalized("td/telegram/MessagesManager.h");
