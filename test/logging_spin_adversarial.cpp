...
TEST(LoggingSpinAdversarial, TsLogSpinLoopMustContainBackoffHintUnderContention) {
...
TEST(LoggingSpinAdversarial, TsLogMustNotUseVoidLockAcquisitionEntryPoint) {
  auto header = load_repo_text("tdutils/td/utils/TsLog.h");
  auto normalized = normalize_for_contract(header);

  ASSERT_TRUE(normalized.find("voidenter_critical();") == td::string::npos);
  ASSERT_TRUE(normalized.find("boolenter_critical();") != td::string::npos);
