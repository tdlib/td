...
TEST(BusinessGuestMessageIntegration, RequiredBusinessGuestGuardPatternsExistInMessagesManager) {
...
TEST(BusinessGuestMessageIntegration, LegacyBusinessGuestUnsafePatternsAreAbsentInMessagesManager) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/MessagesManager.cpp");
  auto normalized = normalize_for_contract(source);
  ...
  const std::vector<td::Slice> forbidden = {
      ...
      "guest_bot_candidate.guest_bot_is_bot=td_->user_manager_->is_user_bot(m->sender_user_id);if(note_guest_bot_top_"
      "dialog_use(guest_bot_candidate,last_guest_bot_message_date)){",
