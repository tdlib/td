...
TEST(ReplyAndUsernameContract, DraftMessageParseClearsSameChatYetUnsentReplyReference) {
...
TEST(ReplyAndUsernameContract, CreateMessageInputReplyToNullsPointerAfterInvalidReplyRejection) {
...
TEST(ReplyAndUsernameContract, CreateMessageInputReplyToRejectPathClearsMessageIdBeforeUnknownServerReplyAllowance) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/MessagesManager.cpp");
  auto normalized = normalize_for_contract(source);
...
TEST(ReplyAndUsernameContract, RestoreReplyToMessageKeepsYetUnsentTargetsOnlyForForwards) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/MessagesManager.cpp");
  auto normalized = normalize_for_contract(source);
...
TEST(ReplyAndUsernameContract, CheckDialogUsernameMapsOccupiedAndPurchasableErrorsDeterministically) {
