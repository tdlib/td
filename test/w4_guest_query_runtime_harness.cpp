...
TEST(W4GuestQueryRuntimeHarness, RejectsNonPositiveIdentifierBeforeAnyConversion) {
...
TEST(W4GuestQueryRuntimeHarness, ConvertsReferenceMessagesBeforeMainMessageAndDispatchesOnce) {
  td::vector<td::telegram_api::object_ptr<td::telegram_api::Message>> references;
  ...
  auto result = td::dispatch_guest_query_qts_update(
      ...
      [&](td::telegram_api::object_ptr<td::telegram_api::Message> &&message)
          -> td::td_api::object_ptr<td::td_api::message> {
        events.push_back(message->get_id() == td::telegram_api::messageEmpty::ID && events.empty()
                             ? HarnessEvent::ConvertReference
                             : HarnessEvent::ConvertMain);
        return make_message_object();
      },
      [&](td::int64 query_id, td::td_api::object_ptr<td::td_api::message> &&message,
          td::vector<td::td_api::object_ptr<td::td_api::message>> &&reference_messages) {
  ...
  ASSERT_EQ(static_cast<int>(HarnessEvent::ConvertReference), static_cast<int>(events[0]));
  ASSERT_EQ(static_cast<int>(HarnessEvent::ConvertMain), static_cast<int>(events[1]));
...
TEST(W4GuestQueryRuntimeHarness, SkipsNullReferenceMessagesButStillDispatchesMainMessage) {
  td::vector<td::telegram_api::object_ptr<td::telegram_api::Message>> references;
...
TEST(W4GuestQueryRuntimeHarness, FailsClosedWhenMainMessageConversionReturnsNull) {
  td::vector<td::telegram_api::object_ptr<td::telegram_api::Message>> references;
  ...
  auto result = td::dispatch_guest_query_qts_update(
      ...
      [&](td::telegram_api::object_ptr<td::telegram_api::Message> &&message)
        ...
        static_cast<void>(message);
        convert_calls++;
        events.push_back(convert_calls == 1 ? HarnessEvent::ConvertReference : HarnessEvent::ConvertMain);
        if (convert_calls == 2) {
          return nullptr;
        }
        return make_message_object();
