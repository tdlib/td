...
TEST(SessionBulkThresholdMaxRuntimeIntegration, QueryOneByteBelowMaximumThresholdStaysInteractive) {
  ...
  connection.send_query(td::BufferSlice(td::string(kMaxValidBulkThresholdBytes - 1, 'q')), false,
                        td::mtproto::MessageId(), {}, false);
...
TEST(SessionBulkThresholdMaxRuntimeIntegration, QueryAtMaximumThresholdEscalatesToBulkData) {
  ...
  connection.send_query(td::BufferSlice(td::string(kMaxValidBulkThresholdBytes, 'q')), false, td::mtproto::MessageId(),
                        {}, false);
  connection.flush(&callback);
