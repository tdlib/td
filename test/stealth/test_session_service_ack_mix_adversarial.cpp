...
TEST(SessionServiceAckMixAdversarial, StateInfoRequestWithAckFloodUsesBulkDataHint) {
  ...
  auto base_message_id = (static_cast<td::uint64>(td::Time::now_cached() * (static_cast<td::uint64>(1) << 32)) | 1u);
  raw_ptr->add_scripted_step(
      make_single_packet_step(make_ack_container_packet(auth_data.get_session_id(), base_message_id, 2048)));
  ...
  connection.get_state_info(td::mtproto::MessageId(td::uint64{1}));
