...
TEST(SessionRecordSizeSequenceHardeningAdversarial,
  ...
  raw_ptr->add_scripted_step(
      make_single_packet_step(make_ack_container_packet(auth_data.get_session_id(), base_message_id, 9000)));
  ...
  connection.send_query(td::BufferSlice(td::string(256, 'r')), false, td::mtproto::MessageId(), {}, false);
  connection.get_state_info(td::mtproto::MessageId(td::uint64{2}));
  connection.flush(&callback);
  ...
  ASSERT_EQ(TrafficHint::Interactive, raw_ptr->sent_hints[2]);
}
...
TEST(SessionRecordSizeSequenceHardeningAdversarial, AckOverflowDrainRecoversPureKeepaliveOnNextFlush) {
  ...
  ASSERT_EQ(TrafficHint::BulkData, raw_ptr->sent_hints[0]);
  ASSERT_EQ(TrafficHint::BulkData, raw_ptr->sent_hints[1]);
  ASSERT_EQ(TrafficHint::Keepalive, raw_ptr->sent_hints[2]);
}
