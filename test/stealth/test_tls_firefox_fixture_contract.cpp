...
TEST(FirefoxFixtureContract, Firefox148EchMetadataMatchesReviewedLinuxFixtureAcrossSeeds) {
  for (auto seed : kQuickSeeds) {
    ...
    ASSERT_EQ(kFirefoxLinuxDesktopReferenceEch.outer_type, hello.ech_outer_type);
    ASSERT_EQ(kFirefoxLinuxDesktopReferenceEch.kdf_id, hello.ech_kdf_id);
    ASSERT_EQ(kFirefoxLinuxDesktopReferenceEch.aead_id, hello.ech_aead_id);
    ASSERT_EQ(kFirefoxLinuxDesktopReferenceEch.enc_length, hello.ech_declared_enc_length);
    ASSERT_EQ(kFirefoxLinuxDesktopReferenceEch.enc_length, hello.ech_actual_enc_length);
    ASSERT_EQ(kFirefoxLinuxDesktopReferenceEch.payload_length, hello.ech_payload_length);
  }
}
...
TEST(FirefoxFixtureContract, Firefox149MacosKeepsDistinctReviewedEchMetadataFromFirefox148) {
  for (auto seed : kQuickSeeds) {
    ...
    ASSERT_EQ(firefox149_macos26_3Ech.outer_type, firefox149.ech_outer_type);
    ASSERT_EQ(firefox149_macos26_3Ech.kdf_id, firefox149.ech_kdf_id);
    ASSERT_EQ(firefox149_macos26_3Ech.aead_id, firefox149.ech_aead_id);
    ASSERT_EQ(firefox149_macos26_3Ech.enc_length, firefox149.ech_declared_enc_length);
    ASSERT_EQ(firefox149_macos26_3Ech.enc_length, firefox149.ech_actual_enc_length);
    ASSERT_EQ(firefox149_macos26_3Ech.payload_length, firefox149.ech_payload_length);
