...
td::Status flush_response_into_tls_init(TlsInit &tls_init, td::SocketFd &peer_fd, td::Slice response) {
  ...
  TRY_RESULT(read_size, tls_init.fd_.flush_read());
  (void)read_size;
  return td::Status::OK();
}
...
TEST(TlsInitResponseLeftoverIntegration, SuccessfulHelloResponsePreservesTrailingTlsRecordForNextLayer) {
  ...
  ASSERT_EQ(trailing_record.size(), tls_init.fd_.input_buffer().size());

  auto buffered = tls_init.fd_.input_buffer().clone().move_as_buffer_slice().as_slice().str();
  ASSERT_EQ(trailing_record, buffered);
}
