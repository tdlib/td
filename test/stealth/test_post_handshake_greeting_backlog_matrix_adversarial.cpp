...
constexpr std::array<size_t, 3> kDelayedInteractiveSizes = {480, 520, 900};
...
double leave_delayed_interactive_backlog(DecoratorFixture &fixture, int write_budget = 1) {
  ...
  for (auto payload_size : kDelayedInteractiveSizes) {
    enqueue_packet(fixture, payload_size, TrafficHint::Interactive, false);
  }
...
size_t count_flushed_delayed_payloads(const std::vector<td::string> &payloads) {
  size_t count = 0;
  for (auto delayed_size : kDelayedInteractiveSizes) {
    if (find_payload_index(payloads, delayed_size) != payloads.size()) {
      count++;
    }
  }
  return count;
...
TEST(PostHandshakeGreetingBacklogMatrixAdversarial,
     DelayedInteractiveBacklogMatrixPreservesArrivalOrderAndGreetingToDrsTargets) {
  constexpr std::array<MatrixCase, 2> kCases = {{
      {"alternating_single_write",
       {TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive,
        TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive},
       {1, 1, 1, 1, 1, 1, 1, 1}},
      {"alternating_zero_budget_prefix",
       {TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive,
        TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive},
       {0, 1, 1, 0, 1, 1, 0, 1}},
  }};
  ...
  for (const auto &test_case : kCases) {
    ...
    auto wakeup = leave_delayed_interactive_backlog(fixture);
    ASSERT_EQ(1, fixture.inner->write_calls);
    ASSERT_EQ(td::string(300, 'x'), fixture.inner->written_payloads[0]);
    ...
    for (size_t cycle = 0; cycle < test_case.budgets.size(); cycle++) {
      auto payload_size = static_cast<size_t>(60 + cycle * 7);
      auto hint = test_case.storm_hints[cycle];
      enqueue_packet(fixture, payload_size, hint, hint == TrafficHint::Keepalive);
      fixture.inner->writes_per_flush_budget_result = test_case.budgets[cycle];
      fixture.decorator->pre_flush_write(fixture.clock->now());

      if (count_flushed_delayed_payloads(fixture.inner->written_payloads) == kDelayedInteractiveSizes.size()) {
        break;
      }
    }

    for (size_t guard = 0; guard < 8 && count_flushed_delayed_payloads(fixture.inner->written_payloads) !=
                                            kDelayedInteractiveSizes.size();
         guard++) {
      auto hint = (guard % 2 == 0) ? TrafficHint::AuthHandshake : TrafficHint::Keepalive;
      enqueue_packet(fixture, static_cast<size_t>(140 + guard * 3), hint, hint == TrafficHint::Keepalive);
      fixture.inner->writes_per_flush_budget_result = 1;
      fixture.decorator->pre_flush_write(fixture.clock->now());
    }

    auto first_index = find_payload_index(fixture.inner->written_payloads, kDelayedInteractiveSizes[0]);
    auto second_index = find_payload_index(fixture.inner->written_payloads, kDelayedInteractiveSizes[1]);
    auto third_index = find_payload_index(fixture.inner->written_payloads, kDelayedInteractiveSizes[2]);

    ASSERT_TRUE(first_index != fixture.inner->written_payloads.size());
    ASSERT_TRUE(second_index != fixture.inner->written_payloads.size());
    ASSERT_TRUE(third_index != fixture.inner->written_payloads.size());
    ASSERT_TRUE(first_index < second_index);
    ASSERT_TRUE(second_index < third_index);
    ASSERT_EQ(960, fixture.inner->per_write_padding_targets[first_index]);
    ASSERT_EQ(1400, fixture.inner->per_write_padding_targets[second_index]);
    ASSERT_EQ(1400, fixture.inner->per_write_padding_targets[third_index]);
  }
}

TEST(PostHandshakeGreetingBacklogMatrixAdversarial,
     ReadyBacklogWakeupStaysImmediateAcrossAlternatingBypassStormMatricesUntilAllDelayedWritesFlush) {
  constexpr std::array<MatrixCase, 2> kCases = {{
      {"keepalive_auth_zero_budget_mix",
       {TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake,
        TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake},
       {0, 1, 0, 1, 0, 1, 1, 1}},
      {"auth_keepalive_sparse_budget_mix",
       {TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive,
        TrafficHint::AuthHandshake, TrafficHint::Keepalive, TrafficHint::AuthHandshake, TrafficHint::Keepalive},
       {0, 0, 1, 0, 1, 0, 1, 1}},
  }};
  ...
  for (const auto &test_case : kCases) {
    ...
    for (size_t cycle = 0; cycle < test_case.budgets.size(); cycle++) {
      auto payload_size = static_cast<size_t>(90 + cycle * 5);
      auto hint = test_case.storm_hints[cycle];
      enqueue_packet(fixture, payload_size, hint, hint == TrafficHint::Keepalive);
      fixture.inner->writes_per_flush_budget_result = test_case.budgets[cycle];
      fixture.decorator->pre_flush_write(fixture.clock->now());

      auto flushed_count = count_flushed_delayed_payloads(fixture.inner->written_payloads);
      if (flushed_count == kDelayedInteractiveSizes.size()) {
        break;
      }
    ...
    for (size_t guard = 0; guard < 8 && count_flushed_delayed_payloads(fixture.inner->written_payloads) !=
                                            kDelayedInteractiveSizes.size();
         guard++) {
      auto hint = (guard % 2 == 0) ? TrafficHint::Keepalive : TrafficHint::AuthHandshake;
      enqueue_packet(fixture, static_cast<size_t>(180 + guard * 5), hint, hint == TrafficHint::Keepalive);
      fixture.inner->writes_per_flush_budget_result = 1;
      fixture.decorator->pre_flush_write(fixture.clock->now());
      if (count_flushed_delayed_payloads(fixture.inner->written_payloads) != kDelayedInteractiveSizes.size()) {
        ASSERT_TRUE(fixture.decorator->get_shaping_wakeup() <= fixture.clock->now() + 1e-6);
    ...
    ASSERT_EQ(kDelayedInteractiveSizes.size(), count_flushed_delayed_payloads(fixture.inner->written_payloads));
  }
}
