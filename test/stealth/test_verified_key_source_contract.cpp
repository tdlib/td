...
TEST(SourceParityContract, BundledMainAndTestPemBlocksMatchPinnedSlots) {
  auto source = read_text_file("td/telegram/net/PublicRsaKeySharedMain.cpp");

  auto main_fingerprint =
      compute_legacy_rsa_public_key_fingerprint(extract_pem_literal(source, "CSlice legacy_main_public_key_pem()"));
  auto test_fingerprint =
      compute_legacy_rsa_public_key_fingerprint(extract_pem_literal(source, "CSlice legacy_test_public_key_pem()"));

  ASSERT_EQ(td::StaticCatalog::pinned_slot(td::mtproto::VaultKeyRole::MainMtproto), main_fingerprint);
  ASSERT_EQ(td::StaticCatalog::pinned_slot(td::mtproto::VaultKeyRole::TestMtproto), test_fingerprint);
}

TEST(SourceParityContract, BundledRecoveryPemBlockMatchesPinnedSlot) {
  auto source = read_text_file("td/telegram/ConfigManager.cpp");
  auto recovery_fingerprint = compute_legacy_rsa_public_key_fingerprint(
      extract_pem_literal(source, "CSlice legacy_simple_config_public_key_pem()"));

  ASSERT_EQ(td::StaticCatalog::pinned_slot(td::mtproto::VaultKeyRole::SimpleConfig), recovery_fingerprint);
}
