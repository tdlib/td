...
constexpr size_t kAckFloodThresholdCount = 1024;
...
TEST(SessionInboundAckContainerDriftIntegration, ParserDrivenAckFloodUsesBulkDataHintThroughFlushPacket) {
  ...
  raw_ptr->add_scripted_step(make_single_packet_step(
      make_ack_container_packet(auth_data.get_session_id(), base_message_id, kAckFloodThresholdCount - 1)));
...
TEST(SessionInboundAckContainerDriftIntegration,
  ...
  raw_ptr->add_scripted_step(make_single_packet_step(
      make_ack_container_packet(auth_data.get_session_id(), base_message_id, kAckFloodThresholdCount - 1)));
  raw_ptr->add_scripted_step(make_single_packet_step(
      make_ack_container_packet(auth_data.get_session_id(), base_message_id + 0x1000, kAckFloodThresholdCount - 1)));
  raw_ptr->add_scripted_step(make_single_packet_step(
      make_ack_container_packet(auth_data.get_session_id(), base_message_id + 0x2000, kAckFloodThresholdCount - 1)));
  ...
  ASSERT_EQ(3, raw_ptr->flush_calls_);
  ASSERT_EQ(3u, raw_ptr->sent_hints.size());
  ASSERT_EQ(TrafficHint::BulkData, raw_ptr->sent_hints[0]);
  ASSERT_EQ(TrafficHint::BulkData, raw_ptr->sent_hints[1]);
  ASSERT_EQ(TrafficHint::BulkData, raw_ptr->sent_hints[2]);
}
