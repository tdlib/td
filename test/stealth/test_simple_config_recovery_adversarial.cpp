...
TEST(SimpleConfigRecoveryAdversarial, CiphertextMutationFailsClosed) {
  td::string data =
...
TEST(SimpleConfigRecoveryAdversarial, MalformedPayloadLengthFailsClosed) {
  auto status =
      td::decode_simple_config_payload(make_simple_config_payload(7, td::telegram_api::help_configSimple::ID));
  ASSERT_TRUE(status.is_error());
}

TEST(SimpleConfigRecoveryAdversarial, HashMismatchFailsClosedBeforeParsing) {
  auto status =
      td::decode_simple_config_payload(make_payload_with_broken_hash(8, td::telegram_api::help_configSimple::ID));
  ASSERT_TRUE(status.is_error());
}

TEST(SimpleConfigRecoveryAdversarial, ConstructorMismatchFailsClosed) {
