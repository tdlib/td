...
constexpr size_t kContainerAckCap = 8192;
constexpr size_t kRaisedBulkThresholdBytes = 70000;
constexpr size_t kAckFloodBeyondContainerCap = 9000;
...
TEST(SessionAckTruncationRuntimeDriftIntegration, AckOverflowPastContainerCapStillUsesBulkDataHint) {
  ...
  raw_ptr->add_scripted_step(make_single_packet_step(
      make_ack_container_packet(auth_data.get_session_id(), fresh_base_message_id(), kAckFloodBeyondContainerCap)));
