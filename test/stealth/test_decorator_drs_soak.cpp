...
DecoratorFixture make_anti_repeat_fixture() {
  MockRng rng(1);
  ...
  auto decorator =
      StealthTransportDecorator::create(std::move(inner), config, td::make_unique<MockRng>(41), std::move(clock));
  CHECK(decorator.is_ok());
  return {decorator.move_as_ok(), inner_ptr, clock_ptr};
}

DecoratorFixture make_phase_reset_fixture() {
  MockRng rng(2);
  auto config = StealthConfig::default_config(rng);
...
TEST(DecoratorDrsSoak, SameConnectionInteractiveCapsRespectAntiRepeatGuardAcrossFlushes) {
  auto fixture = make_anti_repeat_fixture();
  ...
  ASSERT_TRUE(std::count(caps.begin(), caps.end(), 1200) > 24);
}

TEST(DecoratorDrsSoak, SameConnectionIdleResetRestartsInteractivePhaseAfterSteadyProgress) {
  auto fixture = make_phase_reset_fixture();
