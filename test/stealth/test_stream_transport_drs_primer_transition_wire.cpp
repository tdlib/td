...
DrsPhaseModel make_fixed_phase() {
  DrsPhaseModel phase;
  phase.bins = {{kFixedCap, kFixedCap, 1}};
  phase.max_repeat_run = 1;
  phase.local_jitter = 0;
  return phase;
}

StealthConfig make_primer_transition_config() {
  MockRng rng(1);
  auto config = StealthConfig::default_config(rng);
  config.drs_policy.slow_start = make_fixed_phase();
  config.drs_policy.congestion_open = make_fixed_phase();
  config.drs_policy.steady_state = make_fixed_phase();
  ...
  config.drs_policy.min_payload_cap = kFixedCap;
  config.drs_policy.max_payload_cap = kFixedCap;
  return config;
}
...
void enqueue_flush(td::mtproto::IStreamTransport &transport, size_t payload_size) {
...
TEST(StreamTransportDrsPrimerTransitionWire, SubsequentFlushDropsBackToNonPrimerRecordBound) {
  auto decorator = StealthTransportDecorator::create(
      ...
      make_primer_transition_config(), td::make_unique<MockRng>(7), td::make_unique<MockClock>());
...
TEST(StreamTransportDrsPrimerTransitionWire, IdleResetDoesNotReintroducePrimerSizedRecord) {
  auto clock = td::make_unique<MockClock>();
  auto *clock_ptr = clock.get();
  auto decorator = StealthTransportDecorator::create(
      td::make_unique<ObfuscatedTransport>(static_cast<td::int16>(2), ProxySecret::from_raw(make_tls_secret())),
      make_primer_transition_config(), td::make_unique<MockRng>(11), std::move(clock));
  ASSERT_TRUE(decorator.is_ok());
  auto transport = decorator.move_as_ok();
