...
class ScriptedInboundRawConnection final : public RawConnection {
 public:
  ...
  size_t traffic_bulk_threshold_bytes() const final {
    return kMinValidBulkThresholdBytes;
  }
...
TEST(SessionAckMinThresholdIntegration, AckBurstBelowMinimumBoundaryStaysKeepalive) {
  ...
  raw_ptr->add_scripted_step(make_single_packet_step(make_ack_container_packet(
      auth_data.get_session_id(), fresh_base_message_id(), kAckBoundaryBelowMinimumThreshold)));
...
TEST(SessionAckMinThresholdIntegration, AckBurstAtMinimumBoundaryEscalatesToBulkData) {
  auto socket_pair = create_socket_pair().move_as_ok();
  auto raw_connection =
      td::make_unique<ScriptedInboundRawConnection>(td::BufferedFd<td::SocketFd>(std::move(socket_pair.client)));
  auto *raw_ptr = raw_connection.get();
  ...
  raw_ptr->add_scripted_step(make_single_packet_step(
      make_ack_container_packet(auth_data.get_session_id(), fresh_base_message_id(), kAckBoundaryAtMinimumThreshold)));
