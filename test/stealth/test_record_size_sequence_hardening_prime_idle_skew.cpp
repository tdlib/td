...
TEST(RecordSizeSequenceHardeningPrimeIdleSkew, PrimerCountsTowardRepeatGuardUnderDominantSkew) {
...
TEST(RecordSizeSequenceHardeningPrimeIdleSkew, IdleResetClearsRepeatPressureUnderDominantSkew) {
  auto policy = make_policy();
  DominantBinRng rng;
  DrsEngine drs(policy, rng);
