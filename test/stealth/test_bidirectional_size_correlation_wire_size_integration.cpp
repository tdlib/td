...
StealthConfig make_wire_size_floor_config() {
  MockRng rng(1);
...
StealthConfig make_wire_size_jitter_config() {
  MockRng rng(2);
  auto config = make_wire_size_floor_config();
...
td::string make_server_response_wire(td::AesCtrState &inbound_state, td::Slice payload, size_t frame_padding_bytes,
                                     bool include_change_cipher_spec = true) {
  td::string frame(payload.size() + 4 + frame_padding_bytes, '\0');
  auto payload_size = static_cast<td::uint32>(payload.size() + frame_padding_bytes);
  ...
  if (frame_padding_bytes != 0) {
    td::MutableSlice(frame.data() + 4 + payload.size(), frame_padding_bytes).fill('z');
  }
...
td::string make_server_quick_ack_wire(td::AesCtrState &inbound_state, td::uint32 quick_ack,
                                      bool include_change_cipher_spec = true) {
  td::string frame(4, '\0');
...
td::uint32 read_client_quick_ack(ClientEndpoint &client) {
  ...
  LOG(FATAL) << "client failed to decode inbound quick ack";
  UNREACHABLE();
}
...
TEST(BidirectionalSizeCorrelationWireSizeIntegration, PaddedTinyInboundFrameDoesNotArmNextOutboundFloor) {
  ...
  auto inbound_wire = make_server_response_wire(client.inbound_state, td::Slice(tiny_response), 512);
  client.input_writer.append(td::Slice(inbound_wire));
  client.input_reader.sync_with_writer();
  auto inbound_response = read_client_message(client);
  ASSERT_EQ(tiny_response.size() + 512u, inbound_response.size());
  ASSERT_EQ(td::Slice(tiny_response), inbound_response.as_slice().substr(0, tiny_response.size()));
  auto inbound_lengths = extract_tls_record_lengths(td::Slice(inbound_wire));
  ASSERT_EQ(1u, inbound_lengths.size());
  ASSERT_TRUE(inbound_lengths[0] > 192u);
...
TEST(BidirectionalSizeCorrelationWireSizeIntegration, PaddedTinyInboundFrameDoesNotArmPostResponseJitter) {
  ...
  auto wakeup = client.transport->get_shaping_wakeup();
  ASSERT_TRUE(wakeup == 0.0 || wakeup <= client.clock->now());
}
...
TEST(BidirectionalSizeCorrelationWireSizeIntegration, QuickAckDoesNotArmNextOutboundFloor) {
  ...
  auto inbound_wire = make_server_quick_ack_wire(client.inbound_state, kQuickAck);
  client.input_writer.append(td::Slice(inbound_wire));
  client.input_reader.sync_with_writer();
...
TEST(BidirectionalSizeCorrelationWireSizeIntegration, QuickAckDoesNotClearArmedFloor) {
  ...
  auto small_response_wire = make_server_response_wire(client.inbound_state, td::Slice(tiny_response), 0);
  client.input_writer.append(td::Slice(small_response_wire));
  client.input_reader.sync_with_writer();
  auto inbound_response = read_client_message(client);
  ASSERT_EQ(td::Slice(tiny_response), inbound_response.as_slice());

  constexpr td::uint32 kQuickAck = 0x80000021u;
  auto quick_ack_wire = make_server_quick_ack_wire(client.inbound_state, kQuickAck, false);
...
TEST(BidirectionalSizeCorrelationWireSizeIntegration, FramedSizeThresholdBoundaryIsHonored) {
  ...
  write_and_flush_client(arming_client, td::Slice(warmup_request));
  auto arming_warmup_wire = take_available_wire(arming_client.output_reader);
  ASSERT_FALSE(arming_warmup_wire.empty());
  auto threshold_wire = make_server_response_wire(arming_client.inbound_state, td::Slice(tiny_response), 188);
  arming_client.input_writer.append(td::Slice(threshold_wire));
  arming_client.input_reader.sync_with_writer();
  auto threshold_response = read_client_message(arming_client);
  ASSERT_EQ(192u, threshold_response.size());
  write_and_flush_client(arming_client, td::Slice(next_request));
  auto arming_outbound_wire = take_available_wire(arming_client.output_reader);
  auto arming_lengths = extract_tls_record_lengths(td::Slice(arming_outbound_wire));
  ASSERT_EQ(1u, arming_lengths.size());
  ASSERT_EQ(1200u, arming_lengths[0]);

  write_and_flush_client(non_arming_client, td::Slice(warmup_request));
  auto non_arming_warmup_wire = take_available_wire(non_arming_client.output_reader);
  ASSERT_FALSE(non_arming_warmup_wire.empty());
  auto above_threshold_wire = make_server_response_wire(non_arming_client.inbound_state, td::Slice(tiny_response), 189);
  non_arming_client.input_writer.append(td::Slice(above_threshold_wire));
  non_arming_client.input_reader.sync_with_writer();
  auto above_threshold_response = read_client_message(non_arming_client);
  ASSERT_EQ(193u, above_threshold_response.size());
  write_and_flush_client(non_arming_client, td::Slice(next_request));
  auto non_arming_outbound_wire = take_available_wire(non_arming_client.output_reader);
  auto non_arming_lengths = extract_tls_record_lengths(td::Slice(non_arming_outbound_wire));
  ASSERT_EQ(1u, non_arming_lengths.size());
  ASSERT_EQ(320u, non_arming_lengths[0]);
}
