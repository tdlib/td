...
StealthConfig make_unknown_hint_greeting_config(double jitter_ms) {
  MockRng rng(1);
...
struct Harness final {
  ...
  static Harness create(double jitter_ms = 0.0) {
    Harness harness;
    auto inner = td::make_unique<RecordingTransport>();
    harness.inner = inner.get();
    auto clock = td::make_unique<MockClock>();
    harness.clock = clock.get();
    auto decorator = StealthTransportDecorator::create(std::move(inner), make_unknown_hint_greeting_config(jitter_ms),
                                                       td::make_unique<MockRng>(89), std::move(clock));
  ...
  void write_and_flush(td::Slice payload) {
...
TEST(BidirectionalSizeCorrelationUnknownHintGreeting, GreetingEmissionConsumesArmedFloorBeforeDrsResumes) {
  auto harness = Harness::create();

  harness.write_and_flush(td::Slice("first"));
  ...
  harness.write_and_flush(td::Slice("second"));
  ASSERT_EQ(420, harness.inner->stealth_record_padding_targets.back());

  harness.write_and_flush(td::Slice("third"));
  ASSERT_EQ(320, harness.inner->stealth_record_padding_targets.back());
}

TEST(BidirectionalSizeCorrelationUnknownHintGreeting, GreetingWriteConsumesPostResponseJitterBeforeDrsResumes) {
  ...
  td::BufferWriter second(td::Slice("second"), harness.transport->max_prepend_size(),
                          harness.transport->max_append_size());
  harness.transport->write(std::move(second), false);
  auto greeting_wakeup = harness.transport->get_shaping_wakeup();
  ASSERT_TRUE(greeting_wakeup > harness.clock->now());
  ASSERT_TRUE(greeting_wakeup - harness.clock->now() >= 0.011 - 1e-6);
  harness.flush_ready();
  ASSERT_EQ(420, harness.inner->stealth_record_padding_targets.back());

  td::BufferWriter third(td::Slice("third"), harness.transport->max_prepend_size(),
                         harness.transport->max_append_size());
  harness.transport->write(std::move(third), false);
  auto drs_wakeup = harness.transport->get_shaping_wakeup();
