...
TEST(WindowCountAdversarial, LowCountMatrixNeverReachesPinnedLookup) {
  for (size_t count = 0; count < td::mtproto::AuthKeyHandshake::minimum_server_entry_count(); count++) {
...
TEST(WindowCountAdversarial, ValidCountMatrixStillDelegatesToPinnedLookup) {
  for (size_t count = td::mtproto::AuthKeyHandshake::minimum_server_entry_count(); count <= 8; count++) {
    td::net_health::reset_net_monitor_for_tests();
