...
TEST(GuestBotTopDialogRuntime, AcceptsMonotonicGuestBotUsageAndAdvancesWatermark) {
...
TEST(GuestBotTopDialogRuntime, RejectsEqualOrOlderGuestBotUsageFailClosed) {
  auto candidate = make_candidate();
...
TEST(GuestBotTopDialogRuntime, RejectsForwardedOrMismatchedGuestUsageFailClosed) {
  auto candidate = make_candidate();
  td::int32 last_message_date = 0;
...
TEST(GuestBotTopDialogRuntime, ForwardedGuestMessagesRemainEligibleForStandardRouting) {
  auto candidate = make_candidate();

  ASSERT_TRUE(td::should_short_circuit_standard_top_dialog_routing(candidate.guest_bot_via_dialog_id, false));
  ASSERT_FALSE(td::should_short_circuit_standard_top_dialog_routing(candidate.guest_bot_via_dialog_id, true));
  ASSERT_FALSE(td::should_short_circuit_standard_top_dialog_routing(td::DialogId(), false));
}

TEST(GuestBotTopDialogRuntime, RejectsNonBotOrNonUserGuestSendersFailClosed) {
  auto candidate = make_candidate();
  td::int32 last_message_date = 0;
