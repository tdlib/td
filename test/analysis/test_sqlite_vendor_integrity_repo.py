...
class SqliteVendorIntegrityRepoTest(unittest.TestCase):
    def test_vendor_manifest_exists_and_matches_current_rebased_vendor_metadata(
        self,
    ) -> None:
        manifest_path = REPO_ROOT / VENDOR_MANIFEST_RELATIVE_PATH
        self.assertTrue(
            manifest_path.exists(), msg=f"missing vendor manifest: {manifest_path}"
        )
        ...
        self.assertEqual("v4.16.0", manifest["vendor"]["release_tag"])
        self.assertEqual("3.53.1", manifest["vendor"]["sqlite_version"])
        self.assertRegex(manifest["vendor"]["source_tarball_sha256"], r"^[0-9a-f]{64}$")
        self.assertEqual(
            report["sqlite_source_id"], manifest["vendor"]["sqlite_source_id"]
        )
        self.assertEqual(
            report["compile_definitions"], manifest["build"]["compile_definitions"]
        )
        self.assertEqual(
            "tools/sqlite/generate_tdsqlite_rename.py",
            manifest["generation"]["rename_generator"],
        )
        self.assertEqual(
            "tools/sqlite/update_vendor.py", manifest["generation"]["update_script"]
        )
        self.assertIn("sqlite/upstream/sqlite3.c", manifest["hashes"]["sha256"])
        self.assertIn(
            "sqlite/generated/tdsqlite_rename.h", manifest["hashes"]["sha256"]
        )
    ...
    def test_update_vendor_script_exists(self) -> None:
        ...
        self.assertTrue(
            script_path.exists(), msg=f"missing vendor update script: {script_path}"
        )
