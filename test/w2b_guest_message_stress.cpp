...
TEST(W2BGuestMessageStress, RepeatedSourceReadsKeepGuestGuardInvariantsStable) {
  ...
  for (int i = 0; i < kIterations; i++) {
    ...
    ASSERT_NE(td::string::npos,
              normalized.find("if(!is_business_message&&dialog_id==my_dialog_id&&(sender_user_id!=my_id||"
    ...
    ASSERT_NE(td::string::npos,
              normalized.find("autodialog_message=create_message(td_,parse_telegram_api_message(td_,"
                              "std::move(message),false,true,\"get_business_message_message_object\"),"
