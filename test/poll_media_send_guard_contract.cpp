...
TEST(PollMediaSendGuardContract, SendGateRejectsMediaBearingPollsFailClosed) {
  ...
  ASSERT_TRUE(normalized.find("if(message_content_poll_has_media(content,td)){returnStatus::Error(400,"
                              "\"Pollswithmediacan'tbesentyet\");}") != td::string::npos);
}

TEST(PollMediaSendGuardContract, ForwardCopyGateRejectsMediaBearingPollsFailClosed) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/MessageContent.cpp");
  ...
  ASSERT_TRUE(normalized.find("if(message_content_poll_has_media(content,td)){returnfalse;}") != td::string::npos);
}

TEST(PollMediaSendGuardContract, PollManagerInputMediaGuardsRejectPollStateMedia) {
  ...
  auto has_input_media_normalized = normalize_for_contract(has_input_media_region);
  ASSERT_TRUE(has_input_media_normalized.find("if(!get_poll_file_ids(poll_id).empty()){returnfalse;}") !=
              td::string::npos);
  ...
  auto get_input_media_normalized = normalize_for_contract(get_input_media_region);
  ASSERT_TRUE(get_input_media_normalized.find("if(!get_poll_file_ids(poll_id).empty()){") != td::string::npos);
  ASSERT_TRUE(get_input_media_normalized.find("returnnullptr;") != td::string::npos);
}
