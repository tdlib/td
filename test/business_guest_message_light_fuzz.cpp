...
TEST(BusinessGuestMessageLightFuzz, RandomizedProbeOrderKeepsRequiredGuardPatternsPinned) {
  ...
  const std::array<td::string, 12> required = {
      ...
      "if(!is_guest_message){td->messages_manager_->fix_message_topic(dialog_id,message.get(),false);}",
      ...
      R"(automessage_object=get_guest_message_object(std::move(message),true);)",
      ...
      "guest_bot_candidate.is_forward=is_forward;",
...
TEST(BusinessGuestMessageLightFuzz, RandomizedProbeOrderKeepsLegacyUnsafePatternsAbsent) {
  auto source = td::mtproto::test::read_repo_text_file("td/telegram/MessagesManager.cpp");
  auto normalized = normalize_for_contract(source);

  const std::array<td::string, 16> forbidden = {
      ...
      R"(automessage_object=get_business_message_message_object(std::move(message));)",
      R"(get_business_message_message_object(std::move(reply_to_message)))",
      R"(td_api::object_ptr<td_api::message>MessagesManager::get_business_message_message_object()",
      "guest_bot_candidate.guest_bot_is_bot=td_->user_manager_->is_user_bot(m->sender_user_id);"
      "if(note_guest_bot_top_dialog_use(guest_bot_candidate,last_guest_bot_message_date)){",
