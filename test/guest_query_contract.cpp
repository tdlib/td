...
TEST(GuestQueryContract, TdApiSchemaMustExposeGuestQuerySurface) {
...
TEST(GuestQueryContract, TelegramApiSchemaMustExposeGuestTransportSurface) {
...
TEST(GuestQueryContract, GuestQueryEntryPointsMustBeDeclared) {
...
TEST(GuestQueryContract, GuestQueryBotOnlyEntryPointsMustFailClosed) {
  ...
  ASSERT_NE(td::string::npos,
            inline_queries_manager.find(
                "autoinline_message_id=InlineQueriesManager::get_inline_message_id(std::move(ptr));"));
  ASSERT_NE(td::string::npos,
            inline_queries_manager.find("if(inline_message_id.empty()){returnon_error(Status::Error(500,"
...
TEST(GuestQueryContract, GuestQueryIdentifiersMustBeStrictlyPositiveAcrossEntryPoints) {
  auto inline_queries_manager = read_normalized("td/telegram/InlineQueriesManager.cpp");
...
TEST(GuestQueryContract, GuestQueryIdentifiersMustBeRejectedBeforeGuestMessageConversion) {
  auto guest_query_qts_update = read_normalized("td/telegram/GuestQueryQtsUpdate.h");
