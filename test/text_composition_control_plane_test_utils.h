...
namespace td::w5_text_composition_control_plane_test {
...
inline td::string read_premium_cpp() {
  return td::mtproto::test::read_repo_text_file("td/telegram/Premium.cpp");
}

inline td::string read_td_api_tl() {
...
inline td::string normalized_premium_cpp() {
  return normalize_for_contract(read_premium_cpp());
}

inline td::string normalized_td_api_tl() {
  return normalize_for_contract(read_td_api_tl());
}
