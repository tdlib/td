...
| Flow-cadence аномалия к одному destination (S18) | Высокая | Высокая | Capture-driven policy: bounded connect-rate, reuse windows, anti-churn/anti-single-socket guardrails |
| Длительная SNI/domain concentration (S19) | Высокая | Высокая | Domain-set strategy + sticky rotation policy + smoke checks по destination concentration |
...
1. **Все тесты PR-A..PR-10 зелёные**, включая:
