...
Критическое уточнение по итогам ревизии кода (`td/mtproto/TlsInit.cpp`):
...
- Текущее состояние (`td/mtproto/TlsInit.cpp`): non-Darwin ветка использует `Op::permutation`, где extension-блоки перемешиваются глобально через `Random::shuffle(parts)`.
