...
> 2. **S4 (КРИТИЧЕСКОЕ):** по реальному коду `td/mtproto/stealth/TlsHelloBuilder.cpp` ECH type уже `0xFE0D` (не `0xFE02`). В плане это фиксируется как regression-guard и route-aware policy (RU default-off), а не как «новая фича».
...
> 12. **S6/S24 (HIGH):** текущий `Op::permutation + Random::shuffle(parts)` в non-Darwin шире наблюдаемых browser-family ограничений. План фиксирует переход к fixture-bounded order windows (profile-specific), а не к глобальному полному shuffle.
> 13. **S12/S13 (HIGH, уточнение 2026-04-05):** модель «`QTimer::singleShot` + равномерный jitter `5..30ms` + always-coalesce pending buffer» полезна как идея (искажать IPAT/размеры), но неприемлема как production-default: дискретная 1ms-сетка и узкий диапазон формируют detector-visible comb-pattern; без строгих bypass/latency guardrails ломается keepalive/auth cadence. В плане зафиксирован capture-driven sampler (log-normal + truncated Pareto + Markov), bounded ring без sync-bypass и coalescing только в flush-batch по policy.
...
### Аудит предложения из чата: "JA3 fullstring вместо hardcoded ClientHello"

Исходная идея:

1. отдельный `MTClientHelloParser`;
2. внутри него generator по `(domain, browser_type)`;
3. выбор "нужного JA3" через `BrowserProfileRegistry`;
4. перевод JA3 -> hex + patch domain/SNI и browser-specific полей;
5. возврат готового ClientHello.

Вердикт: направление про registry/builder/parser правильное, но связка "JA3 fullstring -> hex -> patch" непригодна как production source-of-truth для TLS wire-мимикрии.

| Пункт предложения | Вердикт | Корректировка в плане |
|---|---|---|
| 1. `MTClientHelloParser` как главный конструктор hello | 🔴 Не принимать | Parser должен быть read-only валидатором/инспектором bytes. Генерация ClientHello остаётся в builder (`td/mtproto/stealth/TlsHelloBuilder.cpp`) + profile policy. |
| 2. Generator внутри parser | 🔴 Не принимать | Нарушение SRP: parser и generator разделяются. Вводится явная цепочка: `BrowserProfileRegistry -> ClientHelloBuilder -> WireParser/Validator`. |
| 3. Registry выбирает JA3 как первичный профиль | 🔴 Не принимать | JA3 использовать только как secondary compatibility signal. Source-of-truth в registry: полный профиль (cipher suites, extension set/order bounds, ALPN, ALPS, ECH mode, PQ policy, padding policy, invariants). |
| 4. "JA3 -> hex" + post-build patcher | 🔴 Не принимать | JA3 не кодирует extension bodies/length graph (ECH payload/enc lengths, key_share bytes/lengths, signature lists, supported_versions, ALPN payload, padding zero-body rules RFC7685). Post-build patching повышает риск length drift и synthetic сигнатур. |
| 5. Возврат "готовой структуры" без parser-gate | 🟡 Частично | Возвращать bytes + metadata, затем обязательный parser-gate: RFC8446 structural validity, ECH length invariants, registry-consistency checks, JA3/JA4 + non-JA3 checks. |

Ключевые причины отклонения "JA3 fullstring как генератора":

- JA3 сжимает ClientHello до списка идентификаторов и теряет критичные поля wire-формата; этого недостаточно для browser-faithful replay.
- По реальным TCP dump в `docs/Samples/Traffic dumps` baseline многосемейный; один "целевой JA3" как runtime-конструктор создаёт новый искусственный кластер и ухудшает правдоподобие.
- По bundled uTLS (`docs/Samples/utls-code`) важны не только ID extension, но и способ сериализации payload/длин (ECH/ALPS/padding semantics).
- По RFC 7685 `padding(21)` должен содержать только нули и добавляться policy-driven (window semantics), а не через ad-hoc patch после сборки.
- По `docs/Samples/xray-core-code` (finalmask/sudoku) маскирование реализовано как stream wrapper, а не как post-build мутатор TLS handshake bytes; перенос модели "patch ready bytes" в TDLib wire-path как default не допускается.

Итоговая архитектура для этого плана (исправленная):

`BrowserProfileRegistry (snapshot/capture-driven) -> ClientHelloBuilder (single source serializer) -> MTClientHelloParser (wire validation + smoke checks) -> runtime policy gate (route-aware ECH, profile/device coherence)`.
...
2. На текущем состоянии `tdlib-obf` ECH type в `td/mtproto/stealth/TlsHelloBuilder.cpp` уже `0xFE0D` (legacy `0xFE02` в этом файле отсутствует). Поэтому для этого репозитория это regression-guard, а не «новый фикс в этом PR».
...
6. По итогам текущего цикла внешние iOS literal-правки фиксируются как plan-only input: в `td/mtproto/stealth/TlsHelloBuilder.cpp` не вносятся точечные ad-hoc изменения вне профильного PR-трека (`PR-2` registry + fixture-driven tests).
...
| S23 | 🔴 CRITICAL | Profile policy | OS/profile incoherence: desktop Linux/Windows не должны внезапно выглядеть как Safari/iOS-профили. Иначе возникает кросс-слойное противоречие (TCP/OS fingerprint vs TLS-family). | Исправить |
...
- `test_logs.pcapng`: выраженно многосемейный набор. Доминирует `JA3=424f6d...` (66 CH) с ECH+PQ, без ALPS и с фиксированным extension-order; присутствуют также семейства с `ALPS=0x44CD`. `0xFE02` найден в 6 TCP ClientHello (единая JA4-family `t13d1516h2_8daaf6152771_d1e053f3eb1c`, SNI `telemost.yandex.ru`), и декодируется как реальный extension-блок (len 174/206), а не как parser/noise artifact.
...
- `Permutation` + GREASE primitives уже есть и полезны как база, но в текущем `td/mtproto/stealth/TlsHelloBuilder.cpp` non-Darwin путь делает глобальный shuffle extension-блоков. Для production default нужен fixture-bounded порядок: профильно-ограниченные окна перестановок и fail-closed проверка against allowed-order set; «полный random shuffle всего списка» не считается браузерно-реалистичным default.
...
| Принцип | Применение |
|---|---|
| **Strict Activation Gate** | Единственная точка включения stealth-shaping/decorator: `create_transport()` и только для `ObfuscatedTcp && secret.emulate_tls()==true`. Дополнительные `emulate_tls` ветки вне transport-shaper (например, запуск `TlsInit` в `ConnectionCreator`) не заменяют этот gate и не добавляют дублирующий shaping-branch в hot path. |
...
| **Pre-sampled Context** | В `TlsHelloContext` pre-sample делается только для handshake-полей (`padding`, `ECH` и др. ClientHello-параметров). Runtime timing/record shaping (IPT/DRS) сэмплируется отдельно в shaper/DRS engine по capture-policy; это не часть `TlsHelloContext`. |
...
| **Bounded Ring + Hard Backpressure** | `write()` никогда не пишет в `inner_` в обход IPT/DRS. При достижении high watermark `can_write()` возвращает `false`, отправка откладывается до drain ring. Никаких sync-overflow bypass и никаких per-write one-shot timer flush. |
...
4. На текущем состоянии сериализация synthetic ClientHello уже вынесена в `td/mtproto/stealth/TlsHelloBuilder.cpp`, а `TlsInit.cpp` использует этот builder как entrypoint. Поэтому PR-A не должен повторно "выносить" builder; задача PR-A — закрепить test seams и parser-gate вокруг существующего serializer без изменения wire-format production пути.
...
- использовать уже существующий serializer/builder из `td/mtproto/stealth/TlsHelloBuilder.cpp` как единый production source для тестов (без дублирования логики в test helper'ах);
...
- проблема не только в `Op::ech_payload()`, но и в том, что он вызывается внутри process-wide singleton `static TlsHello result = []{...}();`;
- если stealth path продолжает использовать `TlsHello::get_default()` для ECH-enabled профилей, per-connection entropy фактически отсутствует даже при частичных правках;
- smoke-gate обязан проверять variance в рамках **одного процесса и одного profile lane**, иначе смешивание разных процессов/профилей может скрыть регрессию.
...
Критичный audit-check после любого рефактора `td/mtproto/stealth/TlsHelloBuilder.cpp`:
...
- текущий production wire-формат в `td/mtproto/stealth/TlsHelloBuilder.cpp` (для regressions).
...
- Chrome131/Chrome120: fixture-driven (в текущем bundled uTLS snapshot — `0x4469`)
- Chrome133+ профили: fixture-driven (в bundled uTLS snapshot — `0x44CD`)
...
Extension-order policy (audit-fix для пункта «Extension-order realism»):

- Текущее состояние (`td/mtproto/stealth/TlsHelloBuilder.cpp`): non-Darwin ветка использует `Op::permutation`, где extension-блоки перемешиваются глобально через `Random::shuffle(parts)`.
- Это шире, чем browser-family ограничения из capture/uTLS и создаёт лишнюю сигнатуру «слишком свободного» порядка.
- Для `Chrome*` профилей policy должна быть bounded и snapshot-driven: допустимы только перестановки внутри разрешённого окна, совместимые с `ShuffleChromeTLSExtensions` в bundled uTLS (не shuffle для GREASE/padding/pre_shared_key инвариантов).
- Для `Firefox*`, `Safari*`, `IOS*`, `Android*` и legacy capture-families default policy — `FixedFromFixture` (без глобального shuffle).
- PR-9 smoke обязан fail-closed: extension-order hash каждого сэмпла должен входить в allow-set конкретного профиля/fixture-family.
...
// Darwin: Chrome131=45, Chrome120=15, Safari26_3=30, Firefox148=10.
// Non-Darwin: Chrome131=65, Chrome120=20, Safari26_3=0, Firefox148=15.
...
BrowserProfile pick_profile_sticky(const ProfileWeights &w,
  ...
  if (platform.desktop_os == DesktopOs::Darwin) {
    if (roll < w.darwin.chrome131) return BrowserProfile::Chrome131;
    if (roll < w.darwin.chrome131 + w.darwin.chrome120) return BrowserProfile::Chrome120;
    if (roll < w.darwin.chrome131 + w.darwin.chrome120 + w.darwin.safari26_3) {
  ...
  if (roll < w.non_darwin.chrome131) return BrowserProfile::Chrome131;
  if (roll < w.non_darwin.chrome131 + w.non_darwin.chrome120) return BrowserProfile::Chrome120;
  return BrowserProfile::Firefox148;
}
```
...
- в `td/mtproto/stealth/TlsHelloBuilder.cpp` ECH extension (`0xFE0D`) зашит в шаблон default ClientHello и не зависит от route hints;
- в runtime-пути (`td/mtproto/TlsInit.cpp` + `td/telegram/net/ConnectionCreator.cpp`) отсутствуют сущности вида `EchMode/RouteHints/active_policy`; в `wait_hello_response()` нет route-aware fallback/circuit-breaker логики;
...
TEST(ProfileWire, Chrome131UsesFe0dNotFe02) {
  auto h = generate_header_test(BrowserProfile::Chrome131, EchMode::Rfc9180Outer);
...
## 9.0.1 Аудит внешней гипотезы `5..30ms singleShot + pending coalescing`

Рассматриваемая идея (внешний PR-паттерн): складывать encrypted bytes в pending-буфер и выполнять `singleShot` flush с равномерной задержкой `5..30ms`.

Вердикт для этого плана: **цель верная, но реализация в таком виде неприемлема для production-default**.

Почему это рискованно:

1. Дискретный uniform `5..30ms` (26 корзин) при типичном scheduler tick даёт detector-visible «гребёнку» IPAT, что само становится fingerprint.
2. Per-write timer (`singleShot` на каждый enqueue) привязывает wire-cadence к granularity event-loop, создавая staircase-паттерн вместо browser-like burst/idle динамики.
3. Always-coalescing без строгих ограничений по deadline/hint ломает semantics для keepalive/auth и увеличивает tail latency.
4. Overflow-путь в pending-буфере опасен соблазном сделать sync bypass write; это прямо нарушает правило «нет passthrough overflow».

Что фиксируем как production-policy:

1. Задержки только capture-driven: log-normal/truncated Pareto + Markov state, без fixed uniform corridor.
2. `Keepalive`, `AuthHandshake`, `BulkData` — обязательный bypass (`delay=0`) в IPT.
3. Coalescing разрешён только в flush-batch и только для безопасного подмножества (`quick_ack=false`, совместимые hint/deadline/cap).
4. Никаких per-write one-shot timers в стиле `QTimer::singleShot`; используется единый wakeup-contract (`get_shaping_wakeup` + poll-loop).
5. Любой overflow решается только backpressure (`can_write=false`), никогда sync write в `inner_`.
6. Activation gate остаётся строгим: shaping применяется только при `ProxySecret::emulate_tls()==true`.

Scope-guard по RFC 7685: `padding(21)` из `docs/Standards/rfc7685.txt` относится к длине ClientHello и не является обоснованием для post-handshake IPT-jitter.
...
enum class TrafficHint : uint8_t {
  ...
  AuthHandshake= 4,  // Pre-auth/control path (`send_no_crypto`, `!has_salt`) — no fixed packet-count heuristic
};
```
...
TEST(IptController, InteractiveSeriesDoesNotCollapseToUniformFiveToThirtyMsComb) {
  MockRng rng(17);
  IptController ipt(IptParams{}, rng);

  std::vector<uint64_t> intervals_us;
  intervals_us.reserve(20000);
  for (int i = 0; i < 20000; i++) {
    auto d = ipt.next_delay_us(true, TrafficHint::Interactive);
    if (d > 0) {
      intervals_us.push_back(d);
    }
  }

  // Regression guard против simplistic модели "uniform integer 5..30ms".
  EXPECT_FALSE(matches_uniform_integer_window(intervals_us,
                                              /*lo_ms=*/5,
                                              /*hi_ms=*/30,
                                              /*bucket_ms=*/1,
...
static bool StealthParamsLoader::validate(const StealthParamsOverride &p) noexcept {
  ...
  if (!validate_phase(p.drs.steady_state)) return false;
        ...
        int desktop_darwin_sum = p.weights.desktop_darwin.chrome131 + p.weights.desktop_darwin.chrome120 +
          p.weights.desktop_darwin.safari26_3 + p.weights.desktop_darwin.firefox148;
        if (desktop_darwin_sum != 100) return false;

        int desktop_non_darwin_sum = p.weights.desktop_non_darwin.chrome131 + p.weights.desktop_non_darwin.chrome120 +
...
PR-9 — это не «один скрипт на JA3», а fail-closed smoke-stage из 4 проверок: fingerprint, IPT, DRS, flow-behavior.
...
Политика использования JA3 (обязательно):
...
- Любой verdict в PR-9 опирается минимум на: JA3/JA4 + структурные инварианты ClientHello (extensions/order/pq/alps/ech policy).
...
3. Агрегация JA3/JA4 и структурных инвариантов разрешена только по fixture'ам, прошедшим `allowed_tags`.
...
```json
{
  ...
  "fixtures": {
    "fx_ch131_0001": {
      ...
      "source_sha256": "9de4ace31cdcf539bad9b94fa4c30af8ece57be8dec1d19d3f54be0302236156",
    ...
    "fx_ch_classic_0007": {
      ...
      "source_sha256": "036dbf64c44e23a33fb3527faa449b4c03c441c566a6952a523034efb3cbae98",
      "source_kind": "browser_capture",
    ...
    "fx_utls_ch120_0001": {
      ...
      "source_sha256": "539e821a53eda1ab6acbfd1661d2ba0a9f8c87a27938709a85b7b69788971648",
  ...
  "profiles": {
    "Chrome131": {
      "include_fixture_ids": ["fx_ch131_0001"],
      "allowed_tags": {
        "source_kind": ["browser_capture", "utls_snapshot"],
        "family": ["chrome131_like"],
        "platform_class": ["desktop"],
        "tls_gen": ["tls13"]
      },
    ...
    "Chrome120": {
      "include_fixture_ids": ["fx_utls_ch120_0001"],
      "allowed_tags": {
        "source_kind": ["browser_capture", "utls_snapshot"],
        "family": ["chrome120_like"],
        "platform_class": ["desktop"],
    ...
    "Firefox148": {
      "include_fixture_ids": ["fx_utls_ff148_0001"],
      "allowed_tags": {
        "source_kind": ["browser_capture", "utls_snapshot"],
        "family": ["firefox148_like"],
        "platform_class": ["desktop"],
    ...
    "Safari26_3": {
      "include_fixture_ids": ["fx_utls_sf263_0001"],
      "allowed_tags": {
        "source_kind": ["browser_capture", "utls_snapshot"],
        "family": ["safari26_3_like"],
        "platform_class": ["desktop"],
    ...
    "IOS14": {
      "include_fixture_ids": ["fx_utls_ios14_0001"],
      "allowed_tags": {
        "source_kind": ["browser_capture", "utls_snapshot"],
        "family": ["ios14_like"],
        "platform_class": ["mobile"],
    ...
    "Android11_OkHttp": {
      "include_fixture_ids": ["fx_utls_and11ok_0001"],
      "allowed_tags": {
        "source_kind": ["browser_capture", "utls_snapshot"],
        "family": ["android11_okhttp_like"],
        "platform_class": ["mobile"],
...
- перед вычислением profile fingerprints строится `resolved_fixture_set = include_fixture_ids`;
...
- если в наборе fixtures профиля более одного `family` или `source_kind` и это не разрешено guard-политикой -> FAIL;
- если fixture-набор профиля содержит конфликтующие ALPS/ECH ожидания между family lane (например, `0x4469` и `0x44CD` без явного family split) -> FAIL для release-gating;
- профиль с пустым `include_fixture_ids` или с набором только advisory fixtures не может быть release-gating profile.
...
Текущий статус в этом файле:

- `Chrome131` использует реальный fixture id (`fx_ch131_0001`);
- `Chrome120`, `Firefox148`, `Safari26_3`, `IOS14`, `Android11_OkHttp` используют явные `utls_snapshot` fixture ids (`fx_utls_*`) как bootstrap.
- Профили, опирающиеся только на `snapshot_only` fixtures, считаются advisory до добавления хотя бы одного `browser_capture` fixture с `trust_tier=verified`.

Нормативный release-контракт:

1. В release-registry запрещены placeholder fixture ids.
2. Для каждого fixture обязателен `source_sha256` и валидный provenance-tag набор.
3. Профиль может быть release-gating только если его `include_fixture_ids` содержит минимум один `browser_capture` fixture с `trust_tier=verified`.
4. Профиль, состоящий только из `snapshot_only` fixtures, автоматически помечается advisory и исключается из release verdict.
...
BATCH_CHECKS = [
  ...
  # check_server_hello_matrix uses parsed ServerHello artifacts from the same scenario.
    ...
    ("No JA3-only decision path", check_no_ja3_only_verdict_path),
]
```
...
def check_ipt(pcap_file: str, baseline_files: list[str]) -> bool:
    ...
    Scope:
      ...
      - derive timing from TLS-record events, not raw TCP segment boundaries (TSO/GSO-safe);
    ...
    FAIL:
      ...
      - interactive IPAT collapses to near-uniform integer buckets in [5ms,30ms]
        (mass_in_window >= 70% and chi-square does not reject uniform on 1ms bins);
...
Примечание: exact tuples заполняются из fixture-парсинга (pcap + snapshot), без ручного literal drift.
...
def check_server_hello_matrix(samples, registry):
    # full parse -> tuple extraction -> allowlist check
...
| `td/mtproto/TlsInit.cpp` | `wait_hello_response` переводится с fixed-prefix на parser-path (PR-10); runtime orchestration вокруг существующего builder entrypoint | PR-10 |
| `td/mtproto/stealth/TlsHelloBuilder.cpp` | Клиентский hello-path остаётся единым serializer source; для PR-A/PR-1/PR-2 добавляются test seams, profile-policy wiring и regression guards; в PR-1 добавляются `Type::EchPayload`, `Type::EchEncKey`, `Type::PqGroupId`, `Type::PqKeyShare` и удаляется static `ech_payload()` | PR-A/PR-1/PR-2 |
...
| Глобальный full-shuffle extension order в non-Darwin (`Op::permutation`) выходит за profile/capture окна | Высокая | Высокая | PR-2: `ExtensionOrderPolicy` + fixture-bounded ordering, PR-9: fail-closed allow-set по order hash |
...
| Зависимость от устаревших JA3-листов как ground truth | Средняя | Высокая | JA3 list = advisory IOC only; решения только через registry + JA4 + structural checks |
...
1. **Все тесты PR-A..PR-10 зелёные** (без исключений; production release запрещён до завершения PR-10 parser-track), включая:
  ...
  - `test_context_entropy: LightweightProfileAddsPaddingInsideBoringWindow` ← критический
  - `test_context_entropy: EchPayloadIsNotProcessWideSingleton` ← критический
  ...
  - `test_batch_builder: DoesNotCoalesceQuickAckPackets` ← критический
  - `test_batch_builder: PreservesPacketOrderWhenCoalescing` ← критический
  - `test_tls_init_response_parser: AcceptsApprovedServerHelloLayouts` ← критический
  - `test_tls_init_response_parser: RejectsUnknownServerHelloLayout` ← критический
...
3. **Smoke tests** против `stealth-drs-ipt` ветки telemt (локально):
  ...
  - `check_server_hello.py`: negotiated `(selected_version, cipher_suite, selected_group)` попадает в capture/snapshot-матрицу для выбранного profile family
  - `check_fingerprint.py`: отчёт содержит JA3 raw tuple + hash (GREASE-stripped) и не допускает JA3-only verdict path
...
[ ] Нет fixed uniform IPT-коридора (например, 5..30ms на 1ms-сетке)
[ ] Нет per-write one-shot timer модели (`singleShot`/`QTimer`) для flush scheduling
[ ] Нет packet-count эвристики вида "первые N packet = AuthHandshake"
