...
>
> **scrapy-impersonate / curl_cffi (НОВОЕ V7 — источник эталонных ClientHello):** `docs/Samples/scrapy-impersonate/` использует библиотеку `curl_cffi`, которая реализует TLS-фингерпринты браузеров через BoringSSL с активными Chrome-профилями. `BrowserType` из curl_cffi включает `chrome131`, `chrome133`, `firefox135`, `safari18_0`, `edge131` и другие версии. Это **сильный программный источник** эталонных ClientHello и хороший способ автоматизировать fixture refresh, но не единственный source of truth: точность зависит от pinned версии `curl_cffi`/BoringSSL и корректности локального capture workflow. Поэтому policy для PR-2: использовать curl_cffi **вместе** с живыми pcap и bundled uTLS snapshot, а не вместо них. В текущем workspace скрипт `capture_chrome131.py` уже присутствует; задача PR-2 — не «добавить скрипт», а зафиксировать provenance (`curl_cffi` version, profile id, capture method, target host, date) и встроить этот инструмент в fixture update workflow.
>
...
| S27 | 🟠 HIGH | curl_cffi capture workflow | Программный capture tool уже есть (`docs/Samples/scrapy-impersonate/capture_chrome131.py`), но baseline всё ещё не фиксирует provenance/version pinning и не встраивает curl_cffi captures в обязательный fixture-refresh workflow рядом с pcap/uTLS snapshot. | Интегрировать в PR-2 |
