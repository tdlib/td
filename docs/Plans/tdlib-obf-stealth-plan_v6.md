...
- ключ включает destination/SNI, device_class и time-bucket (например, сутки), чтобы не было "прыжков" профиля между соседними соединениями;
...
static bool StealthParamsLoader::validate(const StealthParamsOverride &p) noexcept {
  ...
  if (!validate_phase(p.drs.steady_state)) return false;
    ...
    int desktop_sum = p.weights.desktop.chrome131 + p.weights.desktop.chrome120 +
            p.weights.desktop.safari26_3 + p.weights.desktop.firefox148;
    if (desktop_sum != 100) return false;
...
| Mobile/Desktop profile class drift | Высокая | Высокая | Platform hints + class guardrails + smoke check `profile_matches_device_class(...)` |
